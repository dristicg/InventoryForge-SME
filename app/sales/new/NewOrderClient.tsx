...
import { NewSalesOrder, SalesOrderItem } from "../../../types/sales";

interface NewOrderClientProps {
  availableProducts: Product[];
}

export default function NewOrderClient({ availableProducts }: NewOrderClientProps) {
  ...
  const handleAddLineItem = () => {
    ...
    const newItem: SalesOrderItem = {
      ...
      quantity: quantity,
      rate: selectedProduct.selling_price,
      total_price: selectedProduct.selling_price * quantity,
      product: selectedProduct // attach for UI display
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!customerName || lineItems.length === 0) {
      toast.error("Please fill in customer details and add at least one line item.");
      return;
    }

    setIsSubmitting(true);
    
    const orderData: NewSalesOrder = {
      customer_name: customerName,
      total_amount: grandTotal,
      sales_order_items: lineItems.map(item => ({
    ...
    <form onSubmit={handleSubmit} className="space-y-8 pb-12">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/sales" className="rounded-full p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition-colors">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">New Sales Order</h1>
            <p className="text-sm text-slate-500">Draft a new B2B sales quotation.</p>
          </div>
        </div>
        <div className="flex gap-3">
          <Link href="/sales" className="px-5 py-2.5 text-sm font-semibold text-slate-700 bg-white border border-slate-300 rounded-xl hover:bg-slate-50 transition-colors">
            Cancel
          </Link>
          <button
            type="submit"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden p-6">
            ...
            <div>
              <label className="block text-sm font-medium leading-6 text-slate-900">Customer Name / Company</label>
              <div className="mt-2">
                <input
                  type="text"
                  required
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-6">
              ...
              <div className="p-4 bg-blue-50/50 rounded-xl border border-blue-100 mb-6">
                <h3 className="text-sm font-semibold text-blue-900 mb-3">Add Product</h3>
                <div className="flex flex-col sm:flex-row gap-4 items-end">
                  
                  {/* Searchable Dropdown */}
                  <div className="flex-1 relative w-full">
                    <label className="block text-xs font-medium text-slate-700 mb-1">Search SKU or Name</label>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (selectedProduct && e.target.value !== selectedProduct.name) {
                            setSelectedProduct(null); // Reset choice if typing again
                          }
                        }}
                        ...
                        {filteredProducts.length > 0 ? (
                          filteredProducts.map((p) => (
                            <li
                              ...
                              className="relative cursor-pointer select-none py-2 pl-3 pr-9 text-slate-900 hover:bg-blue-600 hover:text-white group"
                            >
                              <div className="flex items-center justify-between">
                                <span className="block truncate font-medium">{p.name}</span>
                                <span className="block truncate text-slate-500 group-hover:text-blue-200">${p.selling_price?.toFixed(2)}</span>
                              </div>
                            </li>
                          ))
                        ) : (
                  ...
                  <div className="w-full sm:w-32">
                    ...
                    <input
                      ...
                      className="block w-full rounded-lg border-0 py-2 text-slate-500 bg-slate-100 ring-1 ring-inset ring-slate-200 sm:text-sm cursor-not-allowed"
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto inline-flex items-center justify-center rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-2 disabled:opacity-50 transition-colors"
                  >
              ...
              <div className="space-y-3">
                {lineItems.map((item) => (
                  <div key={item.product_id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-xl border border-slate-200 bg-white shadow-sm gap-4">
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                      <div className="h-10 w-10 bg-slate-50 rounded-lg flex items-center justify-center text-slate-400 flex-shrink-0">
                        <Package className="h-5 w-5" />
                      </div>
                      <div>
                        <p className="text-sm font-bold text-slate-900">{item.product?.name}</p>
                        <p className="text-xs text-slate-500">{item.quantity} units @ ${item.rate.toFixed(2)}</p>
                      </div>
                    </div>
                ...
                {lineItems.length === 0 && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-slate-500">Cart is empty. Add products to draft the order.</p>
                  </div>
                )}
              </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-slate-900 rounded-2xl shadow-lg border border-slate-800 p-6 sticky top-6">
            ...
            <p className="text-xs text-slate-500 text-center mt-4">Orders begin in the "Quotation" stage.</p>
          </div>
        </div>
      </div>
    </form>
